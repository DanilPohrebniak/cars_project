...
class LoginForm:
    def __init__(self, width, height):
        ...
        self.title = self.font.render("Register Form", True, (255, 255, 255))
        self.text = ""
        ...
        self.register_button = pygame.Rect(325, 400, 140, 45)
        self.username = ""
        self.password = ""
    ...
    def handle_events(self):
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if self.register_button.collidepoint(pygame.mouse.get_pos()):
                    if self.username and self.password:
                        if self.user.create_user(self.username, self.password):
                            mw.main_window(self.username)
                        else:
                            self.text = "This username is already taken."
                            self.login_error = self.font.render(self.text, True, (255, 102, 102))
                            error_rect = self.title.get_rect(center=(220, 90))
                            self.screen.blit(self.login_error, error_rect)
                            pygame.display.flip()
                    else:
                        self.text = "Register error, check the entered data"
                        self.login_error = self.font.render(self.text, True, (255, 102, 102))
                        error_rect = self.title.get_rect(center=(220, 90))
                        self.screen.blit(self.login_error, error_rect)
                        pygame.display.flip()
